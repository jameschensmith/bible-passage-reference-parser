...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "et" space "suivant" abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( "et" space "suivant" ) [a-e] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "ch" ( "apitres" / "apitre" / "ap" abbrev? / "s" abbrev? / abbrev? ) ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "v" ( "ersets" / "erset" / "ers" abbrev? / "er" abbrev? / "v" abbrev? / abbrev? ) ) ![a-z] sp
...
sequence_sep
  = ([,;/:&\-\u2013\u2014~] / "." !(sp "." sp ".") / "et" !(space "suivant" ) / "comparer" / space)+
    { return "" }
