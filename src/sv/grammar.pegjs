...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp ( "ff" abbrev? / "f" abbrev? ) abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( ( "ff" abbrev? / "f" abbrev? ) ) [a-e] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "kap" ( "itlen" / "itlet" / "itel" / abbrev? ) ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "v" ( "erser" / abbrev? ) ) ![a-z] sp
...
title
  = (cv_sep / sequence_sep)? val:"rubrik"
