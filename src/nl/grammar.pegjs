...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "en" space "volgende" space "verzen" abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( "en" space "volgende" space "verzen" ) [a-e] ![a-z]
...
v_explicit
  = sp ( "v" ( "erzen" / "ers" / "s" abbrev? / abbrev? ) ) ![a-z] sp
...
sequence_sep
  = ([,;/:&\-\u2013\u2014~] / "." !(sp "." sp ".") / "en" / "vgl" / "zie" space "ook" / space)+
    { return "" }
...
title
  = (cv_sep / sequence_sep)? val:"opschrift"
