...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp ( "ss" / "," space "ecc" / "ecc" ) abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( ( "ss" / "," space "ecc" / "ecc" ) ) [a-d] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "c" ( "apitoli" / "apitolo" / "app" abbrev? / "ap" abbrev? / "c" abbrev? ) ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "v" ( "ersetto" / "ersetti" / "ersi" / "v" abbrev? / abbrev? ) ) ![a-z] sp
...
sequence_sep
  = ([,;/:&\-\u2013\u2014~] / "." !(sp "." sp ".") / "vedi" space "anche" / "vedi" / "cfr" / "e" / space)+
    { return "" }
...
title
  = (cv_sep / sequence_sep)? val:"titolo"
