...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "e" space "sig" abbrev? abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( "e" space "sig" abbrev? ) [a-d] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "cap" ( [íi]i "tulos" / [íi]i "tulo" / "s" abbrev? / abbrev? ) ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "v" ( "ers" [íi]i "culos" / "ers" [íi]i "culo" / "ers" abbrev? / "er" abbrev? / "ss" abbrev? / "s" abbrev? / "v" abbrev? / abbrev? ) ) ![a-z] sp
...
title
  = (cv_sep / sequence_sep)? val:"t" [íi]i "tulo"
