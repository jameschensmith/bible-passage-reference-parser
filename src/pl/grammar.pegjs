...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "nn" abbrev? ![a-z]
    { return {"type": "ff", "value": [val_1], "indices": [peg$savedPos, peg$currPos - 1]} }

next_v
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "n" ! [n] abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( "n" ! [n] / "nn" ) [a-e] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "r" ( "ozdziały"i / "ozdział"i [óo]i "w" / "ozdział"i / "ozdz" / "r" abbrev? / abbrev? ) ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "w" ( "erset" [óo]i "w" / "ersety" / "erset" / "ers" abbrev? / abbrev? ) ) ![a-z] sp
...
title
  = (cv_sep / sequence_sep)? val:"tytuł"i
